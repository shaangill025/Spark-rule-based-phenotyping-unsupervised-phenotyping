...
object T2dmPhenotype {
  ...
  def transform(medication: RDD[Medication], labResult: RDD[LabResult], diagnostic: RDD[Diagnostic]): RDD[(String, Int)] = {
    ...
    /** Find CASE Patients */
    val type1_dm_diag = diagnostic.filter { f => type1_dm_dx(f.code) }.map(_.patientID)
    val type2_dm_diag = diagnostic.filter { f => type2_dm_dx(f.code) }.map(_.patientID)
    val type1_dm_medi = medication.filter { f => type1_dm_med(f.medicine) }.map(_.patientID)
    val type2_dm_medi = medication.filter { f => type2_dm_med(f.medicine) }.map(_.patientID)
    ...
    val date1 = medication.filter(f => type1_dm_med(f.medicine)).map(f => (f.patientID, f.date.getTime)).reduceByKey((x, y) => math.min(x, y))
    val date2 = medication.filter(f => type2_dm_med(f.medicine)).map(f => (f.patientID, f.date.getTime)).reduceByKey((x, y) => math.min(x, y))
    val step1_case3 = date1.join(date2)
    val step2_case3 = step1_case3.filter(f => f._2._1 >= f._2._2).map(_._1)
    ...
    val glu_patients = labResult.filter { f => f.testName.contains("glucose") }.map(_.patientID).distinct()
    val abnormal_patients = labResult.filter { f =>
      val name = f.testName
      val value = f.value
    ...
    val mellitusPatients = diagnostic.filter { f => (mellitus(f.code) || f.code.take(3) == "250") }.map(_.patientID)
    ...
    val phenotypeLabel = sc.union(casePatients.map(f => (f, 1)), controlPatients.map(f => (f, 2)), others.map(f => (f, 3)))
